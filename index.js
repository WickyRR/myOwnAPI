...
app.get('/jokes/:id',(req,res)=>{
  try{
    ...
    const item = jokes.find((joke)=>joke.id === id);
    res.status(200).json(item);
  }catch(error){
    res.status(500).json({message:error.message});
  }
...
app.get('/filter',(req,res)=>{
  try{
    ...
    console.log(type);
    const item = jokes.filter((joke)=>joke.jokeType === type);
    res.status(200).json(item);
  }catch(error){
    res.status(500).json({message:error.message});
  }
...
app.patch('/jokes/:id',(req,res)=>{
  ...
  const existingJokeIndex = jokes.findIndex((joke)=>joke.id === id);
  ...
  jokes[existingJokeIndex] = replacementJoke;
  res.status(200).json(replacementJoke);
});
...
app.delete('/jokes/:id',(req,res)=>{
  ...
  if(jokeIndex>-1){
    ...
    res.status(404).json({error:"Error happend can't delete"});
  }
